...
const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> => {
    if (fields.length < 1 || methods.length < 1){
        return makeFailure("Expression not of the form (class <var>+ <binding>+)");
    }
    ...
    const varDecl = map(makeVarDecl, fields);
    const methodsResult = mapResult(method => parseL31CExp(second(method)), methods[0]);
    const namesOfMethod = map(m => m[0], methods[0]);
    const bindingsResultMethods = bind(methodsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, namesOfMethod, vals)));
    return safe2((fields: VarDecl[], methods : Binding[]) => makeOk(makeClassExp(fields, methods)))
                (makeOk(varDecl), bindingsResultMethods);
}
...
const unparseString = (les: VarDecl[]): string =>
    map((s=>s.var), les).join(" ");
...
const unparseClassExp = (cl: ClassExp) : string => 
    `(class (${unparseString(cl.fields)}) (${map((b: Binding) => `(${b.var.var} ${unparseL31(b.val)})`, cl.methods).join(" ")}))`
