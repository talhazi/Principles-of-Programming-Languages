...
import { concat, chain, join, map, zipWith, append } from "ramda";
...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>{
        ...
        if (!(isArray(varDecls) )) 
            return makeFailure('Malformed fields in "class" expression 1');
        if (!isGoodBindings(bindings)) 
            return makeFailure('Malformed methodss in "class" expression 2');
        const fields : Result<VarDecl>[] = map(parseVarDecl, varDecls);
        const fieldsToVarArray= map((f)=>isOk(f) ? f.value : makeVarDecl("",makeFreshTVar()),fields)
        const methods : Result<Binding[]> = parseBindings(bindings);
        if (!isString(typeName)){
            return makeFailure('Malformed bindings in "class" expression 4');     // TO UPDATE
        }
        return safe3((typeName: TVar, fields: VarDecl[], methods: Binding[]) => makeOk(makeClassExp(typeName, fields, methods)))
            (makeOk(makeTVar(typeName)), makeOk(fieldsToVarArray), methods);
    }
...
// const vals : Result<CExp[]> = mapResult(binding => parseL5CExp(second(binding)), bindings);
// const methods : Result<Binding[]> = bind(vals, (vs: CExp[]) => makeOk(zipWith(makeBinding, vars, vs)));
// return bind(methods, (ms : Binding[]) => makeOk(makeClassExp(typeName,fields,ms)));

// Tal & Joni sol
//     if (!isGoodBindings(bindings)) {
//         return makeFailure('Malformed bindings in "class" expression 1');
//     }
//     if (!isArray(varDecls) || !allT(isString, varDecls)){
//         return makeFailure('Malformed bindings in "class" expression 2');
//     }
//     const varDecl=map((varDecl) =>makeVarDecl(varDecl,makeFreshTVar()),varDecls);
//     const methodsResult = mapResult(binding => parseL5CExp(second(binding)), bindings);
//     const namesOfMethod= map(binding => first(binding), bindings);
//     if(!allT(isString,namesOfMethod))
//         return makeFailure('Malformed bindings in "class" expression 3');
//     const VarDeclsNameOfMethods=map((x)=>makeVarDecl(x,makeFreshTVar()),namesOfMethod)
//     if (!isString(typeName)){
//         return makeFailure('Malformed bindings in "class" expression 4');     // TO UPDATE
//     }
//     const bindingsResultMethods = bind(methodsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, VarDeclsNameOfMethods, vals)));
//     return safe3((typeName: TVar ,Decls: VarDecl[], methods : Binding[]) => makeOk(makeClassExp(typeName,Decls, methods)))
//                 (makeOk(makeTVar(typeName)), makeOk(varDecl), bindingsResultMethods);
// }
...
export const parsedToClassExps = (p: Parsed): ClassExp[] =>  // Parsed == EXP | Program
    isProgram(p) ? SearchProgramHelper(p.exps) : SearchProgramHelper([p]);

    const SearchProgramHelper = (exps:Exp[]): ClassExp[] =>{
        if (isEmpty(exps))
            return [];
        if (isClassExp(exps[0])){
            return append(exps[0],(SearchProgramHelper(rest(exps))));
        }
        return SearchProgramHelper(rest(exps));
    }   
