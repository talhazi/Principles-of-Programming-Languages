...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => 
    isEmpty(A.parsedToClassExps(parsed)) ? E.makeEmptyTEnv() : helper(A.parsedToClassExps(parsed));
    
    const helper = (classes: A.ClassExp[]) : E.TEnv => {
        const TypeNames = R.map((class1) => class1.typeName.var, classes)
        const Generic = R.map((class1) => A.classExpToClassTExp(class1), classes);
        const env = E.makeExtendTEnv(TypeNames, Generic ,E.makeEmptyTEnv());
        return env;
    }
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const constraints=bind(typeofExp(exp.val, tenv),(expVal:T.TExp)=>checkEqualType(expVal,exp.var.texp,exp));
    return bind(constraints,(x:true)=>makeOk(T.makeVoidTExp()));
};
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> => {
    const DecideEnv=A.isDefineExp(exp) ? E.makeExtendTEnv([exp.var.var],[exp.var.texp],tenv) : tenv;
    const isEmptyScenario=typeofExp(exp,DecideEnv);
    return isEmpty (exps) ? isEmptyScenario :
        bind(isEmptyScenario,() => typeofProgramExps(first(exps),rest(exps),DecideEnv));
}
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    if (E.isEmptyTEnv(tenv)){
        return makeFailure("Wrong");
    }
    if (!tenv.vars.includes(exp.var.var)){
        return typeofSet(exp,tenv.tenv);
    }
    const indexOf=tenv.vars.indexOf(exp.var.var);
    const CheckingTypes=bind(typeofExp(exp.val,tenv),(valToCheck)=>checkEqualType(valToCheck,tenv.texps[indexOf],exp))
    return bind(CheckingTypes,(x:true)=>makeOk(T.makeVoidTExp()));
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const namesOfFields=R.map((f)=>f.var ,exp.fields)
    const TExps=R.map((f)=>f.texp ,exp.fields)
    let env=E.makeExtendTEnv(namesOfFields,TExps, tenv);
    const MethodsVals = R.map((m)=> m.val ,exp.methods)  
    const constraintMethods = R.map((m)=> m.var.texp ,exp.methods);
    const constraints = zipWithResult((x, y) => bind(typeofExp(y, env), (v) => 
                                    checkEqualType(x, v, exp)), constraintMethods, MethodsVals);
    return bind (constraints,_=>makeOk(T.makeProcTExp(TExps, A.classExpToClassTExp(exp))));
}